setTimeout(() => {
    ...
    function showYearView(yIndex) {
        ...
        // year auto scroll
        const yearTitle = years[yIndex].querySelector('.year_text');
        if (yearTitle) {
    ...
    function showSubpost(yIndex, sIndex) {
        ...
        if (videoSrc && postVideo) {
            ...
            if (!sub.poster_image || sub.poster_image.trim() === "") {
    ...
    function startAutoplay() {
        stopAutoplay(); // clear any existing autoplay first
        ...
        // always start looping afterwards
    ...
    years.forEach((yearDot, i) => {
        yearDot.querySelector('.year_text').addEventListener('click', () => {
            ...
            if (window.innerWidth < 767) {
                years.forEach(item => item.classList.remove('active', 'show'));
    ...
    document.addEventListener('click', (event) => {
        if (!event.target.closest('.post-content') && !event.target.closest('.years')) {
            document.querySelectorAll('.years').forEach(item => item.classList.remove('active', 'show'));
        }
    });
}, 1000);